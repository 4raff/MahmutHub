...
local OWB_REVERSE = {}
for _, v in pairs(OWB_NAME_MAP) do
    OWB_REVERSE[v] = true
end

local WB_REVERSE = {}
for _, v in pairs(WB_NAME_MAP) do
    WB_REVERSE[v] = true
end
...
function AnchorState:setFarm(cf)
    self.farmCFrame = cf
end
function AnchorState:clearFarm()
    self.farmCFrame = nil
end
function AnchorState:get()
    return self.farmCFrame or self.lastCFrame
end
function AnchorState:clear()
    self.farmCFrame = nil;
    self.lastCFrame = nil
end
...
function SpawnState:set(k, v)
    self._active[k] = v
end
function SpawnState:get(k)
    return self._active[k]
end
function SpawnState:clear()
    self._active = {}
end
...
function NPCProvider.isValidCFrame(cf)
    ...
    local p = cf.Position
    return p.Magnitude >= 1 and p.Y >= -10 and math.abs(p.X) <= 5000 and math.abs(p.Z) <= 5000
end
...
function NPCProvider.getIslandFromEnemy(name)
    ...
    if ENEMY_TO_ISLAND[name] then
        return ENEMY_TO_ISLAND[name]
    end
...
function Combat.equipStyle()
    ...
    -- already correct
    if styleValue == lastStyle and (styleValue == "Melee" or isToolEquipped()) then
        return
    end
...
function Combat.useSkill(isBoss)
    if not (SkillState.autoAll or (SkillState.bossOnly and isBoss)) then
        return
    end
...
local StyleWatchdog = (function()
    ...
    local function forceEquip()
        if tick() - lastReequipTime < COOLDOWN then
            return
        end
    ...
    local function setupListeners()
        ...
        addConn = char.ChildAdded:Connect(function(child)
            if not (child:IsA("Tool") and active and farmingActive) then
                return
            end
            if tick() - lastReequipTime < COOLDOWN then
                return
...
local function fireSea2Remote(bossType, difficulty)
    ...
    if not cfg or not cfg.remote then
        return
    end
    pcall(function()
        cfg.remote:FireServer(difficulty)
    end)
    task.wait(0.5)
end

function BossSpawnerSea2.enable(bossType, difficulty)
    fireSea2Remote(bossType, difficulty)
end
...
function BossSpawnerSea2.onBossTypeChanged(newType)
    ...
    if not diff or not newType then
        return
    end
...
function BossSpawnerSea2.onDifficultyChanged(newDiff)
    ...
    if newDiff == lastSea2Difficulty then
        return
    end
    ...
    if newDiff then
        SpawnState:forEach(function(key, isActive)
            if isActive then
                ...
                if bt and BOSS_SPAWN_CONFIG_SEA2[bt] then
                    pcall(function()
                        BossSpawnerSea2.enable(bt, newDiff)
                    end)
                end
            end
        end)
    ...
    pcall(function()
        ...
        elseif cfg.customRemote then
            ...
            if cfg.customArgs then
                for _, a in ipairs(cfg.customArgs) do
                    table.insert(args, a)
                end
            end
...
function BossSpawnerSea1.onDifficultyChanged(newDiff)
    ...
    if newDiff == lastSea1Difficulty then
        return
    end
    ...
    if newDiff then
        SpawnState:forEach(function(bossType, isActive)
            if isActive and BOSS_SPAWN_CONFIG_SEA1[bossType] then
                ...
                if cfg.hasDifficulty then
                    pcall(function()
                        BossSpawnerSea1.enable(bossType, newDiff)
                    end)
                end
            end
        end)
...
local FarmController = (function()
    ...
    function ctrl:interrupt()
        self._interrupt = true
    end
    function ctrl:clearInterrupt()
        self._interrupt = false
    end
    function ctrl:wasInterrupted()
    ...
    local function attackNPC(npc, isBoss, c)
        ...
        while NPCProvider.isAlive(npc) and tick() < deadline do
            if c:wasInterrupted() then
                break
            end

            local ok2, freshCF = pcall(function()
                return npc:GetPivot()
            end)
    ...
    local function farmLoop(c)
        ...
        while not c:wasInterrupted() do
            ...
            local owbSel = AutoFarmState.everyOWB and "all" or
                               (AutoFarmState.selectedOWB and Options["Select Open World Bosses"] and
                                   Options["Select Open World Bosses"].Value) or nil
            local wbSel = AutoFarmState.everyWB and "all" or
                              (AutoFarmState.selectedWB and Options["Select World Bosses"] and
                                  Options["Select World Bosses"].Value) or nil
            ...
            if bsea2Sel and sea2Diff then
                local sel = type(bsea2Sel) == "string" and {
                    [bsea2Sel] = true
                } or bsea2Sel
                ...
                for bossType in pairs(BOSS_SPAWN_CONFIG_SEA2) do
                    if c:wasInterrupted() or not AutoFarmState.selectedBSea2 then
                        break
                    end
                    if sel[bossType] then
                        local boss = BossSpawnerSea2.findBoss(bossType, sea2Diff)
                        if boss then
                            attackNPC(boss, true, c)
                        end
                    end
                end
            else
            ...
            -- ------------------------------------------------
            if bsea1Sel then
                local sel = type(bsea1Sel) == "string" and {
                    [bsea1Sel] = true
                } or bsea1Sel
                ...
                for bossType, cfg in pairs(BOSS_SPAWN_CONFIG_SEA1) do
                    if c:wasInterrupted() or not AutoFarmState.selectedBSea1 then
                        break
                    end
                    if sel[bossType] then
                        local boss =
                            (cfg.hasDifficulty and sea1Diff) and BossSpawnerSea1.findBoss(bossType, sea1Diff) or
            ...
            if wbSel then
                ...
                for _, npc in ipairs(wbs) do
                    if c:wasInterrupted() or not anyActive() then
                        break
                    end
                    if NPCProvider.isAlive(npc) then
                        attackNPC(npc, true, c)
                    end
                end
                if #wbs == 0 then
                    stayAtAnchor()
            ...
            if (AutoFarmState.selectedEnemies or AutoFarmState.everyEnemy) and not c:wasInterrupted() then
                ...
                if #selectedTypes == 0 then
                    ...
                    for _, prefix in ipairs(selectedTypes) do
                        ...
                        local npcs = NPCProvider.queryNormal(function(p)
                            return p == prefix
                        end)
                        if #npcs > 0 then
                            local ok0, cf0 = pcall(function()
                                return npcs[1]:GetPivot()
                            end)
    ...
    local function setupWaveListener(diffArg)
        ...
        task.spawn(function()
            ...
            if state.active and DungeonWaveVote then
                pcall(function()
                    DungeonWaveVote:FireServer(diffArg)
                end)
            end
        end)
            ...
            elseif data.phase == "cleared" then
                task.spawn(function()
                    ...
                    if DungeonWaveVote then
                        pcall(function()
                            DungeonWaveVote:FireServer(getDiffArg())
                        end)
                    end
                end)
            end
            ...
            while tick() < deadline do
                ...
                if O then
                    ...
                    if dOpt and diOpt and tog then
                        ...
                        if type(dv) == "string" and dv ~= "" then
                            state.dungeon = dv
                        end
                        if type(div) == "string" and div ~= "" then
