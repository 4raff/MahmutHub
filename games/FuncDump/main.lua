...
local function DumpAllScripts()
	...
	print("[FuncDump] Ditemukan " .. #scripts .. " script(s)")
	
	for i, scriptData in pairs(scripts) do
		pcall(function()
			print("[FuncDump] Processing (" .. i .. "/" .. #scripts .. ") - " .. scriptData.path)
			
			local source = scriptData.instance.Source
			if source and #source > 0 then
				scriptData.source = source
				table.insert(results, scriptData)
			end
		end)
	end
...
local function DecompileAllScripts()
	local dumpedScripts = DumpAllScripts()
	local output = ""
	
	for i, scriptData in pairs(dumpedScripts) do
		output = output .. "\n" .. string.rep("=", 80) .. "\n"
		output = output .. "-- Script Path: " .. scriptData.path .. "\n"
		output = output .. "-- Script Type: " .. scriptData.type .. "\n"
		output = output .. string.rep("=", 80) .. "\n\n"
		
		output = output .. scriptData.source .. "\n\n"
	end
	
	return output, dumpedScripts
end
...
return {
	...
	DecompileAllScripts = DecompileAllScripts
}